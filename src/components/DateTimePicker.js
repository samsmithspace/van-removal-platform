...
const DateTimePicker = ({ onDateChange, onTimeChange }) => {
    ...
    useEffect(() => {
        const fetchUnavailableDates = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/driver/getdate`);
    ...
    const handleDateChange = async (newDate) => {
        if (moment(newDate).isValid()) {
            ...
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/driver/available-time-periods`, {
                ...
                if (periods.length > 0) {
                    ...
                    onTimeChange(moment(startTime, 'HH:mm').format('HH:mm'));
